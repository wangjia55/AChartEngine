...
public class ScatterChart extends XYChart {
    ...
    public void drawSeries(Canvas canvas, Paint paint, List<Float> points, List<Double> values,
                           XYSeriesRenderer renderer, SupportSeriesRender supportSeriesRender,
        ...
        switch (renderer.getPointStyle()) {
            ...
            case CIRCLE:
                for (int i = 0; i < length; i += 2) {
                    paint.setColor(getSupportColor(renderer,supportSeriesRender,values.get(i + 1)));
            ...
            case CIRCLE_POINT:
                ...
                for (int i = 0; i < length; i += 2) {
                    paint.setColor(getSupportCirclePointColor(renderer,supportSeriesRender,values.get(i + 1)));
                    drawCirclePoint(canvas, paint, points.get(i), points.get(i + 1));
                }
                paint.setColor(renderer.getColor());
                break;
        }
    ...
    /**
     * 如果曲线支持了颜色分级，获取分级后的颜色数据，这个是给Circle-Point 这种样式提供的方法
     */
    private int getSupportCirclePointColor(XYSeriesRenderer renderer, SupportSeriesRender supportSeriesRender, double value) {
        int color = 0;
        if (supportSeriesRender.isColorLevelValid()) {
            color = supportSeriesRender.getLevelColorByValue(value);
        } else {
            color = renderer.getInnerCircleColor();
        }
        return color;
    }

    /**
     * 如果曲线支持了颜色分级，获取分级后的颜色数据
     */
    private int getSupportColor(XYSeriesRenderer renderer, SupportSeriesRender supportSeriesRender, double value) {
        int color = 0;
        if (supportSeriesRender.isColorLevelValid()) {
            color = supportSeriesRender.getLevelColorByValue(value);
        } else {
    ...
    private void drawCirclePoint(Canvas canvas, Paint paint, float x, float y) {
